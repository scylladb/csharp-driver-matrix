...
class Email:
    ...
    def prepare_email(self, subject, content, recipients, html=True, files=()):  # pylint: disable=too-many-arguments
        ...
        attachment_size = 0
        for path in files:
            attachment_size += Path(path).stat().st_size
            with open(path, "rb") as fil:
                part = MIMEApplication(
                    fil.read(),
                    Name=Path(path).name
                )
            part['Content-Disposition'] = 'attachment; filename="%s"' % Path(path).name
            msg.attach(part)
...
def send_mail(recipients, report):
    loader = jinja2.FileSystemLoader(Path(__file__).resolve().parent / 'report_templates')
    ...
    email_in_file = Path(__file__).parent / "test_results" / "test_results_email.html"
...
def get_driver_origin_remote(csharp_driver_path):
    return check_output(["bash", "-c", "git config --get remote.origin.url"], cwd=csharp_driver_path, text=True).strip()
